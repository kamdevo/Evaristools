...
{
    /**
     * Convert uploaded PowerPoint presentation to PDF
    ...
    public function convert(Request $request)
    {
        try {
            ...
            $request->validate([
                'file' => 'required|mimes:ppt,pptx,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation|max:25600', // Max 25MB
            ]);
            ...
            $fileName = 'ppt_' . time() . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            
            // Store the uploaded file temporarily
            $file->move(storage_path('app/temp'), $fileName);
            $pptFilePath = storage_path('app/temp/' . $fileName);
            ...
            $success = $this->convertWithAutoRotation($pptFilePath, $pdfFilePath);
            ...
            return response()->json([
                'error' => 'No se pudo convertir el archivo PowerPoint a PDF. Verifica que el archivo no esté corrupto o intenta de nuevo.'
            ], 500);

        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json([
                'error' => 'Archivo inválido. Solo se permiten archivos PowerPoint (.ppt o .pptx) de máximo 25MB.'
            ], 422);
            
        } catch (\Exception $e) {
            Log::error('PowerPoint to PDF conversion error: ' . $e->getMessage());
            Log::error('Stack trace: ' . $e->getTraceAsString());
            
            // Clean up temporary files if they exist
            if (isset($pptFilePath) && file_exists($pptFilePath)) {
                unlink($pptFilePath);
            }
            ...
            return response()->json([
                'error' => 'Error al convertir la presentación. Por favor intenta de nuevo.'
            ], 500);
        }
    }
    
    /**
     * Convert PowerPoint to PDF with automatic API rotation
    ...
    private function convertWithAutoRotation($inputPath, $outputPath)
    {
        ...
        // All APIs failed
        Log::error('All PowerPoint conversion APIs failed');
        return false;
    }
    ...
    private function convertWithConvertAPI($inputPath, $outputPath)
    {
        try {
            ...
            curl_setopt_array($curl, [
                CURLOPT_URL => "https://v2.convertapi.com/convert/pptx/to/pdf?Secret={$apiSecret}",
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_POST => true,
            ...
            if ($pdfContent && file_put_contents($outputPath, $pdfContent)) {
                Log::info('ConvertAPI: Conversion successful');
                return true;
            }
    ...
    private function convertWithPDFco($inputPath, $outputPath)
    {
        try {
            ...
            $curl = curl_init();
            curl_setopt_array($curl, [
                CURLOPT_URL => 'https://api.pdf.co/v1/pdf/convert/from/pptx',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_POST => true,
            ...
            if ($pdfContent && file_put_contents($outputPath, $pdfContent)) {
                Log::info('PDF.co: Conversion successful');
                return true;
            }
